...
class View:
    ...
    def setup_player(self):
        try:
            ...
            if self.my_player.get_db_status():
                self.addFavourites.configure(command=self.add_song_to_favourites)
                self.loadFavourites.configure(command=self.load_songs_from_favourites)
                self.removeFavourites.configure(command=self.remove_song_from_favourites)
            else:
        ...
        self.previousButton.configure(command=self.play_previous_song)
        self.playList.configure(font="Vivaldi 12")
        self.playList.bind("<Double-1>", self.list_on_double_click)
    ...
    def add_song(self):
        ...
        self.playList.configure(fg=apply_color)
    ...
    def show_song_details(self):
        ...
        song_min, song_sec = divmod(self.song_length, 60)
        self.songTotalDuration.configure(text=str(song_min) + ":" + str(song_sec))
        self.songTimePassed.configure(text="0:0")
        last_song_index = self.song_name.rfind(".")
        apply_song_name = self.song_name[0:last_song_index]
        if len(apply_song_name) > 14:
            apply_song_name = apply_song_name[0:14] + "..."
        self.songName.configure(text=apply_song_name)

    def play_song(self):
        print("cureent thread ", threading.currentThread())
        if self.isThreadRunning:
            self.stop_threat()
        self.curr_sec = 1
        print("cureent thread ", threading.currentThread())
        print("before ", self.isThreadRunning, self.stopThread)
        ...
        try:
            ...
            self.songProgress.configure(length=self.song_length)
            self.songProgress.configure(maximum=self.song_length)
            print(self.isThreadRunning, self.stopThread)
            self.setup_thread()
    ...
    def list_on_double_click(self, e):
        if self.isThreadRunning:
            self.stop_threat()
        self.play_song()
    ...
    def show_timer(self, total_sec):
        print("thread running show timer ", threading.currentThread())
        if self.curr_sec == 1:
        ...
        while self.curr_sec <= total_sec:
            ...
            time.sleep(1)
            if self.stopThread:
                print("thread dead ", threading.currentThread())
                break
            self.curr_sec += 1
    ...
    def stop_song(self):
        ...
        self.play_song()
        print("stop and place at starting point.")
    ...
    def pause_song(self):
        ...
        if self.isPaused:
            ...
            self.isPaused = False
            print("play from current position.")
        else:
            self.stop_threat()
            self.my_player.pause_song()
            self.isPaused = True
            print("pause at current position.")

    def load_next_song(self):
        next_song_index = self.sel_song_index_tuple[0]
        print("de-select index is ", next_song_index)
        self.playList.select_clear(next_song_index)
        if next_song_index == len(self.playList.get(0, END)) - 1:
            next_song_index = -1
        next_song_index += 1
        print("selected index is. ", next_song_index)
        self.playList.select_set(next_song_index)
        self.play_song()

    def play_previous_song(self):
        print("thread calling previous", threading.currentThread())
        if hasattr(self, "sel_song_index_tuple"):
            try:
                self.sel_song_index_tuple = self.playList.curselection()[0]
            except IndexError:
                return
            print("de-select index is ", self.sel_song_index_tuple)
            self.playList.select_clear(self.sel_song_index_tuple)
            if self.sel_song_index_tuple == 0:
                self.sel_song_index_tuple = len(self.playList.get(0, END))
            self.sel_song_index_tuple -= 1
            print("selected index is. ", self.sel_song_index_tuple)
            self.playList.select_set(self.sel_song_index_tuple)
            self.play_song()
    ...
    def add_song_to_favourites(self):
        ...
        try:
            if len(fav_song_index_tuple) == 0:
                raise NoSongSelectedError("Please select a song to add.")
    ...
    def remove_song_from_favourites(self):
        ...
        try:
            ...
            result = self.my_player.remove_songs_from_favorites(song_name)
            if result.find("No song present") != -1:
                print("This song does not exits.")
                return
            print("removed song : ", song_name)
            if song_name == self.song_name:
                self.my_player.stop_song()
            self.playList.delete(song_index)
